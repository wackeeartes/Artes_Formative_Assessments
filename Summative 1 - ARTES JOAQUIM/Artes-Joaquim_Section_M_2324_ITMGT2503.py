...
import ast
unnecessary_text = ["<P>", "</P>", "<B>", "</B>", "<I>", "</I>"]
unnecessary_punctuation = r"&$@[].,'#()-\"!?’_;:/"

jsondata = str(jsondata).upper()
for x in range(len(unnecessary_text)):
    jsondata = jsondata.replace(str(unnecessary_text[x]),"")

for x in range(len(unnecessary_punctuation)):
    jsondata = jsondata.replace(str(unnecessary_punctuation[x]),"")
...
list_jsondata = list(jsondata)
count = 0

for x in range(len(list_jsondata)):
    if len(list_jsondata[x]) > 3:
        count = list_jsondata.count(list_jsondata[x])
    print(str(list_jsondata[x]) + " " + str(count))
...
while ((F*1000) + (R*600) + (C*750) <= 18000) and ((F*500) + (R*750) + (C*300) <= 12000) and ((F*50) + (R*45) + (C*75) <= 1500):
    F += 1
    R += 1
    C += 1
    F_list.append(F)
    ...
    for f in range(len(F_list)): 
        for r in range(len(R_list)):
            for c in range(len(C_list)):
                value = 10*(F_list[f]) + 12*(R_list[r]) + 16*(C_list[c]) 
...
def Collatz(start_number):
    ...
    dictionary.update([("collatz_sequence",collatz_sequence),("collatz_length",collatz_length),("max_collatz",max_collatz),("sequence_sum",sequence_sum)])
...
def Collatz_winner(number_list):
    ...
    for number in range(len(number_list)):
        yes = Collatz(number_list[number])
